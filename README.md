...
PyPrimer was written in Python3.8+ and runs as a command-line-interface application, requiring the following input files to function:
...
PyPrimer can optionally filter orthogroup candidates using genome wide FST population data for focal species. The original implementation focused on intron variation between  *N. lecontei* and *N. pinetum* from Glover et al. (2024)

PyPrimer can be run as a tutorial using files from the example_file directory on GitHub. Compressed NCBI feature tables and GFF files for four *Neodiprion* and one *Diprion* species are provided along with a file to implment the optional FST filtering between *N. lecontei* and *N. pinetum*.
...
```
```
conda install bioconda::pyprimer-ilps
```
